...
class JenkinsPluginTest < ActiveSupport::TestCase
  ...
  test 'jenkins username has value' do
    assert_equal(SystemTestConfig.jenkins_username, @project_settings_hash[@jenkins_plugin.unique_id][:jenkins_username][:value])
  end

  test 'jenkins password has value' do
    assert_equal(SystemTestConfig.jenkins_password, @project_settings_hash[@jenkins_plugin.unique_id][:jenkins_password][:value])
  end
end